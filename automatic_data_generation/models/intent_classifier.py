...
def intent_classification(emb_dim=100, rnn_type='GRU'):
    ...
    class RNN_classifier(nn.Module):
        def __init__(self, hidden_dim, emb_dim=300, num_linear=3, n_classes=8,
    ...
    nh = 500
    nl = 3
    model = RNN_classifier(nh, emb_dim=emb_dim, n_classes=7,
                           bidirectional=True, rnn_type=rnn_type)
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            'Epoch: {}, Training Accuracy: {:.4f}, Validation Accuracy: {:.4f}'.format(
                epoch, epoch_corrects, val_corrects))
